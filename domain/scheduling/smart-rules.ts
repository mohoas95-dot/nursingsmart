...
//   به هیچ ترکیب خاصی وابسته نباشد و «هر گونه» ۵ شیفت متوالی، مستقل از اینکه از چه
//   الگویی ساخته شده باشد، نقض شمرده شود. رسیدن به ۵ واحد شیفتِ متوالی ممنوع است
//   (حداکثر مجاز ۴ واحد = یک بلوک ۲۴ساعته است).
//
//   نمونه‌ها (همه بر پایهٔ همین یک قانون واحد، بدون استثنای شیفت‌محور):
//     - MEN (روز۱) + M (روز۲) → M,E,N,M = ۱+۱+۲+۱ = ۵ → ممنوع.
//     - MEN (روز۱) + ME (روز۲) → ۱+۱+۲+۱+۱ = ۶ → ممنوع.
//     - N (روز۱) + MEN (روز۲) → ۲+۱+۱+۲ = ۶ → ممنوع.
//     - EN (روز۱) + ME (روز۲) → ۱+۲+۱+۱ = ۵ → ممنوع.
//     - EN (روز۱) + M (روز۲) → ۱+۲+۱ = ۴ → مجاز (معادل همان بلوک ۲۴ساعته).
...
/**
 * حداکثر تعداد واحدهای شیفتِ متوالیِ مجاز؛ رسیدن به ۵ واحد شیفت متوالی ممنوع است.
 * ۴ واحد دقیقاً معادل یک بلوک کاری ۲۴ساعته (مثلاً MEN یا EN+M) است.
 */
 ...
 * همین وزن‌دهی است که قانون را «شیفت‌مستقل» می‌کند: هر ترکیبی که به ۵ واحد متوالی
 * برسد ممنوع است، فارغ از اینکه MEN+M باشد یا EN+ME یا N+MEN.
 */
...
/**
 * زنجیره‌هایی که به ۵ واحد شیفت متوالی یا بیشتر رسیده‌اند (غیرمجاز).
 * این بررسی کاملاً شیفت‌مستقل است: هر ترکیبی (MEN+M، EN+ME، N+MEN، MEN+MEN و …)
 * که مجموع واحدهای متوالی‌اش از ۴ بگذرد، نقض شمرده می‌شود.
 */
